...
score = 0;
function preload() {
    modal = ml5.imageClassifier("DoodleNet");
}
function setup() {
...
function classifyCanvas() {
    modal.classify(canvas, gotResults);
}
function gotResults(error, result) {
    ...
    if (result) {
        console.log(result);
        document.getElementById("art").innerHTML = result[0].label;
        document.getElementById("confidence").innerHTML = result[0].confidence.toFixed(2) + "%";
    }
}
function checkSketch() {
    ...
    if (timer_check == "completed"||answer_holder == "set") {
        ...
        document.getElementById("score").innerHTML = timer_counter;
        updateCanvas();
    }
